...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QGroupBox, QRadioButton, QLineEdit, QFileDialog
)
...
def toggle_auto_start(enable: bool):
    ...
    if enable:
        ...
        with open(vbs_path, "w", encoding="utf-8") as f:
            f.write(f'Set ws = CreateObject("WScript.Shell")\nws.Run "pythonw.exe ""{os.path.join(SCRIPT_DIR, "claude_floating_launcher.py")}""", 0, False\n')
        ps = f'''$ws=New-Object -ComObject WScript.Shell;$sc=$ws.CreateShortcut("{lnk}");$sc.TargetPath="{vbs_path}";$sc.WindowStyle=7;$sc.WorkingDirectory="{SCRIPT_DIR}";$sc.Description="Claude Code 浮窗";$sc.Save()'''
...
def launch_claude_code(config=None):
    ...
    try:
        ...
        try:
            subprocess.Popen(["cmd","/c","start","Claude Code",cmd], cwd=working_dir, creationflags=subprocess.CREATE_NO_WINDOW)
        except Exception:
            subprocess.Popen(["cmd","/c","start",cmd], cwd=working_dir)
