...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # 版本号
        ver = self._label(f"v{VERSION}", size=9, color="#8E8E93")
...
class FloatingWidget(QWidget):
    ...
    def _build_paint_cache(self):
        ...
        border.setColorAt(0.0, QColor(*IOS_BORDER, 190))
        border.setColorAt(0.45, QColor(*IOS_BORDER, 110))
        border.setColorAt(1.0, QColor(*IOS_BORDER, 55))
        self._cache["border"] = border

        # 顶面柔光渐变
        hl = QLinearGradient(0, 0, 0, s * 0.58)
        hl.setColorAt(0.0, QColor(255, 255, 255, 125))
        hl.setColorAt(0.45, QColor(255, 255, 255, 45))
        ...
        inner.setColorAt(0.9, QColor(0, 0, 0, 8))
        inner.setColorAt(1.0, QColor(0, 0, 0, 20))
        ...
        spec.setColorAt(0.25, QColor(255, 255, 255, 55))
    ...
    def paintEvent(self, event):
        ...
        if self.is_hovered:
            border = QLinearGradient(0, 0, 0, s)
            border.setColorAt(0.0, QColor(*IOS_BORDER, int(190 * 1.3)))
            border.setColorAt(0.45, QColor(*IOS_BORDER, int(110 * 1.3)))
            border.setColorAt(1.0, QColor(*IOS_BORDER, int(55 * 1.3)))
            pen = QPen(QBrush(border), 1.0)
        else:
