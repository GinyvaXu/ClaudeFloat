"""构建安装包 v1.4.1: SetupWizard + ClaudeFloat.exe + 图标 → ClaudeFloat_Setup.exe"""
...
if not os.path.exists(APP_EXE):
    print(f"[ERROR] ClaudeFloat.exe not found at {APP_EXE}")
    exit(1)
...
if not os.path.exists(version_file):
    version_file = os.path.join(BASE, "src", "VERSION")
...
print("Building ClaudeFloat_Setup.exe (v1.4.1)...")
PyInstaller.__main__.run(args)

# 5. 清理临时文件
for f in ["setup_wizard_tk.py", "ClaudeFloat.exe", "claude_icon.ico", "claude_icon.png", "VERSION"]:
