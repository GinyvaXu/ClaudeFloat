...
def install():
    ...
    # 使用 pythonw.exe 或 sys.executable
    python_exe = sys.executable if sys.executable else "pythonw.exe"

    # 构造 VBS — 使用字符串拼接而非 .format() 避免花括号崩溃
    vbs = 'Set ws = CreateObject("WScript.Shell")\n'
    vbs += f'ws.Run """{python_exe}"" ""{launcher_path.replace(chr(34), chr(34)+chr(34))}""", 0, False\n'
    try:
    ...
    # 使用环境变量安全传参到 PowerShell，避免注入
    lnk_path = get_lnk_path()
    ps_env = os.environ.copy()
    ps_env["CC_LNK"] = lnk_path
    ps_env["CC_TARGET"] = vbs_path
    ps_env["CC_WORKDIR"] = SCRIPT_DIR
    ps = (
    ...
    try:
        result = subprocess.run(
            ...
            env=ps_env, capture_output=True, text=True, timeout=30
        )
