...
def launch_claude_code(config=None):
    ...
    # 检查 claude 是否已安装（shutil.which 返回完整路径）
    ...
    mode = config.get("launch_mode", "normal")

    # 分离程序路径和参数 — 避免 subprocess.Popen list 形式将含空格字符串整体引号包裹
    if mode == "skip_permissions":
    ...
    _log().info("启动 Claude Code [%s]", mode)
    try:
        subprocess.Popen(
        ...
        # wt 不可用（未安装 Windows Terminal 或 App Execution Alias 不兼容）
        # fallback 到 cmd start — 使用完整路径避免 PATH 查找问题
        _log().info("wt 不可用，使用 cmd start fallback")
        try:
            subprocess.Popen(
                ["cmd", "/c", "start", "Claude Code"] + args,
...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        title_text = self._label("Claude Code 浮窗设置", size=16, color=s["tx"])
        ...
        self.rb_normal.setToolTip(
            "以标准模式启动 Claude Code（命令: claude）\n"
            "执行文件读写、系统命令等敏感操作前会弹出权限确认提示，安全可控"
        )
        self.rb_skip.setToolTip(
            "跳过权限询问模式（命令: claude --dangerously-skip-permissions）\n"
            "Claude Code 可自由执行终端命令、读写文件、访问网络，"
            "且以上操作均不会弹出确认提示\n\n"
        ...
        info_icon.setToolTip(
            "「跳过权限」模式下，Claude Code 可以执行任意终端命令、"
        ...
        warn_container.setMinimumHeight(52)
        ...
        # 退出行为
        self.cb_cleanup = QCheckBox("退出时关闭 Claude Code 进程")
        ...
        self.cb_cleanup.setToolTip("启用后，退出浮窗时将自动结束所有正在运行的 claude.exe 聊天进程")
        ...
        log_hint = self._label(
            f"日志文件: %APPDATA%\\ClaudeFloat\\logs\\ClaudeFloat.log", size=9, color=s["hi"]
        )
        ...
        self._tw = {
            'container': container,
            'groups': [box, box_theme, box2, box3, box_snap, box4],
            ...
            'buttons': [preview_btn, cancel_btn, browse_btn, reset_btn],
            'save_btn': save_btn,
            'close_btn': close_btn,
            'sliders': [self.size_slider, self.op_slider],
        ...
        content_h = max(content_h, 700)
        self.setMinimumSize(420, 700)
        self.resize(420, content_h)
        _log().debug("[设置] 布局计算高度: %dpx (sizeHint=%d)", content_h, outer.sizeHint().height())

        # 居中显示
        screen = QApplication.primaryScreen().geometry()
        self.move((screen.width() - 420) // 2, (screen.height() - content_h) // 2)
...
def main():
    ...
    # ── 崩溃日志：写入应用日志目录 ──
    _crash_dir = os.path.join(_get_config_dir(), "logs")
    os.makedirs(_crash_dir, exist_ok=True)
    CRASH_LOG = os.path.join(_crash_dir, "ClaudeFloat_crash.log")
    try:
        _main()
